...
import { ProductFormSchema, CategoryAttribute } from "@repo/types";
...
const AddProduct = () => {
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      if (!token) {
        message.error("You must be logged in to add a product");
        return;
      }
      ...
      await addProduct({ product: productPayload as never, token }).unwrap();

...
const AddCategory = () => {
  ...
  const handleAddCategory = async (
    ...
    try {
      ...
      if (!token) {
        message.error("You must be logged in to add a category");
        return;
      }
      await addCategory({ category: data, token }).unwrap();
      message.success("Category created successfully");
      form.reset();
    } catch (error) {
      console.error("Failed to add category:", error);
      message.error((error as any).data?.message || "Failed to add category");
    }
  };
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() => append({ key: "", label: "", options: [] })}
                  >
                    Add Attribute
                  </Button>
                </div>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} placeholder="e.g. size, storage" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
